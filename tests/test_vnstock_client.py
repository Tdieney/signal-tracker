"""Unit and adversarial tests for VnstockMarketClient and live Vietnam market data adapter."""
...
from pipeline.providers.vnstock_client import VN30_SYMBOLS, VnstockMarketClient
...
class TestVnstockMarketClient(unittest.TestCase):
    """Test suite verifying VnstockMarketClient behavior, retries, and data normalization."""
    ...
    def test_vn30_symbols_constant_completeness(self):
        """Verify VN30 symbols tuple contains exactly 30 unique, valid ticker symbols."""
        self.assertEqual(len(VN30_SYMBOLS), 30)
        self.assertEqual(len(set(VN30_SYMBOLS)), 30)
    ...
    def test_fetch_daily_bars_successful_response(self):
        """Verify parsing of standard DNSE Entrade OHLCV payload format."""
        mock_payload = {
    ...
    def test_fetch_daily_bars_filters_malformed_and_nonpositive_prices(self):
        """Verify negative or zero prices are strictly dropped from result set."""
        mock_payload = {
            "t": [1787277600, 1787536800],
            "o": [-10.0, 71.0],
            "h": [72.0, 72.5],
            "l": [69.5, 70.8],
            "c": [71.5, 0.0],  # Invalid close
            "v": [5000000, 4500000],
        }
        mock_resp = DummyHTTPResponse(json.dumps(mock_payload).encode("utf-8"))
        self.mock_opener.open.return_value = mock_resp
